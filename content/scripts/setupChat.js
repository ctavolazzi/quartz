...
#chat #ai #week-<% tp.date.now("ww") %> #q<% tp.date.now("Q") %>`;
    ...
    try {
        await app.vault.create('static/templates/Chat Template.md', template);
        return '✅ Chat template created successfully';
    } catch (error) {
        return `❌ Error creating Chat template: ${error.message}`;
    }
};